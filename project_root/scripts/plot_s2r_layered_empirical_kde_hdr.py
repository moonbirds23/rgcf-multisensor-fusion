"""Plot four empirical KDE/HDR longitudinal/lateral error distributions.
...
and left-normal directions of the ground-truth trajectory. Four algorithms are
shown on equal-sized aligned planes; Track-1 is intentionally excluded. No
single-Gaussian fit or mean-centering is applied.
"""
...
def draw_figure(errors: pd.DataFrame, output: Path) -> Dict[str, Path]:
    x_limits, y_limits = _common_limits(errors)
    apply_paper_style()
    ...
    for _, display_name, color, layer in METHODS:
        ...
        grid_x, grid_y, density, dx, dy = estimate_density(
            part["longitudinal_error_m"].to_numpy(float),
            part["lateral_error_m"].to_numpy(float),
            x_limits,
            y_limits,
        )
        threshold_90 = _hdr_threshold(density, dx, dy, 0.90)
        threshold_50 = _hdr_threshold(density, dx, dy, 0.50)
        maximum = float(density.max())
        if not threshold_90 < threshold_50 < maximum:
            raise RuntimeError("Invalid density thresholds for {}.".format(display_name))
        _add_plane(axis, x_limits, y_limits, layer)
        offset = layer + 0.018
        outer = mpl_colors.to_rgba(color, 0.32)
        inner = mpl_colors.to_rgba(color, 0.68)
        axis.contourf(
            grid_x,
            grid_y,
            density,
            zdir="z",
            offset=offset,
            levels=[threshold_90, threshold_50, maximum * 1.001],
            colors=[outer, inner],
            antialiased=True,
        )
        axis.contour(
    ...
    # Internal reference axes intersect at the raw-data zero-error origin.
    axis.plot(
        [x_limits[0], x_limits[1]],
        [0.0, 0.0],
        [0.0, 0.0],
    ...
    region_handles = [
        Patch(facecolor="#687887", edgecolor="none", label="50% empirical HDR"),
        Patch(facecolor="#C4CDD5", edgecolor="none", label="90% empirical HDR"),
    ]
    axis.legend(
